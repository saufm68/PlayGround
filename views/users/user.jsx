  ...
  render() {
    if (this.props.user.list.length > 0) {
      var list = this.props.user.list.map(element => {
        ...
        return (
          <a key={element.id} href={linkUrl}>
            <div
              className="card bg-dark d-inline-block mr-3 mb-3 text-center"
              style={{ width: 160 + "px" }}
            >
              <img
    ...
    if (
      this.props.cookie.loginStatus === this.props.cookie.check &&
      this.props.cookie.userId == this.props.user.id
    ) {
      var editButton = (
        <a className="float-right mt-2 mr-2" href={editUrl}>
          Edit
        </a>
      );
      var deleteButton = (
        <form
          className="deleteButton d-inline-block float-right"
          method="POST"
          action={deleteUrl}
        >
      ...
      <Default cookie={this.props.cookie}>
            ...
            >
              <div className="card-body">
                <div className="profile-title border-bottom-neon mb-3">
                  <h3 className="card-title neon-green d-inline-block mb-1">
        ...
        <div className="row mt-3">
          <div className="col game-list-container ">
            <h3 className="neon-green border-bottom-neon mb-3 m-l-xtra">
              List Of Games
            </h3>
            <div className="gameScroll m-l-xtra">{list}</div>
          </div>
        </div>
      </Default>
    );
