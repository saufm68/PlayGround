...
module.exports = (db, cloudinary) => {
  const uploadGameForm = (request, response) => {
    ...
    if (
      cookie.loginStatus === cookie.check &&
      sha256(SALT + request.cookies["username"] + "loggedin") === cookie.check
    ) {
  ...
  const editForm = (request, response) => {
    db.games.editForm(request.params.id, (error, result) => {
      ...
      response.render("games/editForm", {
        cookie: cookie,
        post: result,
        currentPost: request.params.id
      });
  ...
  const rating = (request, response) => {
    db.games.rating(
      parseInt(request.body.value),
      request.cookies["currentPost"],
      request.cookies["userId"],
      (error, result) => {
        if (error) {
