module.exports = dbPool => {
  ...
  const comments = (input, callback) => {
    ...
    let date = new Date();
    let dt_simplified = `${date.getFullYear()}/${date.getMonth() +
    ...
    dbPool.query(text, values, (error, result) => {
      ...
      let text2 = `SELECT users.username FROM users WHERE id = '${
        comments.user_id
      }';`;
  ...
  const publish = (inputs, uploader, callback) => {
    ...
    let values = [
      ...
      inputs.rating,
      true,
      JSON.stringify(inputs.map),
      inputs.player_function,
      inputs.enemy_function
    ];
    ...
    dbPool.query(text, values, (error, result) => {
      ...
      dbPool.query(text2, (error, result) => {
        ...
        dbPool.query(text3, (error, result) => {
          ...
          let text4 = `UPDATE posts SET rating=${
            passedResult["avg"]
          } WHERE id='${currentPost}';`;
